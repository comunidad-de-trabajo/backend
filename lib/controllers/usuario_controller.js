...
export const getAuth = async (req, res) => {
  ...
  try {
    ...
    Usuario.findOne({
      ...
      .then((usuario) =>
        usuario
          ? res.send({ auth: { isAuthenticated: true, rol: usuario.rol } })
          : res.sendStatus(404)
      )
      .catch(() => res.sendStatus(500));
  } catch (ex) {
...
export const forgotPassword = async (req, res) => {
  const { nueva_contrasenia, contrasenia_confirm } = req.body;

  if (nueva_contrasenia !== contrasenia_confirm)
    return res.status(400).json({ message: 'Los passwords no son iguales' });

  let contraseniaHashed = await bcrypt.hash(nueva_contrasenia, 10);
