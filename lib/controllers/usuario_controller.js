...
export const updateId = async (req, res) => {
  const onSuccess = (Usuario) =>
    Usuario.update(
      ...
      {
        fields: ['nombreUsuario', 'contrasenia', 'rol'],
      }
    )
...
const findTable = (id, { onSuccess, onNotFound, onError }) => {
  Usuario.findOne({
    attributes: ['id', 'nombreUsuario', 'contrasenia', 'rol'],
    where: { id },
  })
...
export const getUserRoleByJWT = async (req, res) => {
  const resolvedToken = jwt.verify(req.body.token, 'secret');
  Usuario.findOne({
    where: { email: resolvedToken.email },
  }).then((usuario) => (usuario ? res.send(usuario.rol) : res.sendStatus(404)));
};
