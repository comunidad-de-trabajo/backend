...
export const uploadImage = async (file, registro) => {
  ...
  const bucket = gc.bucket('comunidad-de-trabajo-empresas');
...
export const createRegistro = async (body) => {
  ...
  try {
    ...
    registro.urlImagen = `https://storage.googleapis.com/comunidad-de-trabajo-empresas/empresas-logos/${registro.id}-${registro.nombreComercial}`;
    await registro.save();
    return registro;
  } catch (err) {
    console.log(err);
